...
module.exports = {
  ...
  incrementVersion: (currentVersion, incrementLevel) => {
    ...
    console.log(`build ${parsedCurrentVersion.build}`)
    let revision
    let version
    if ( parsedCurrentVersion.build && parsedCurrentVersion.build != '' ) {
      revision = Number(String(parsedCurrentVersion.build).split('rev').pop())
      if (!_.isFinite(revision)) {
        throw new Error(`Could not extract revision number from ${currentVersion}`)
      }
      if (revision == 0) {
        version = parsedCurrentVersion.version + '+rev1'
        console.log(`revision0: returning ${version}`)
        return version
      } else {
        version =  parsedCurrentVersion.version + '+rev' + (revision + 1)
